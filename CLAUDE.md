...
- `npm test` — vitest, **1082 tests** across TWO projects, all green as of 2026-07-28. (S19 reflection-search perf had added +54: `engine/__tests__/reflection.test.ts` 36 — the golden equivalence over 18 branch-coverage probe scenes, a brute-force ORACLE that transcribes the pre-S19 algorithm and fuzzes it against the new one over randomized scenes at origins 0/1e3/1e5, the `surfaces.length`-unchanged guard that protects S18's grid cap, and a negative-radius-circle test that forces `isBlocked`'s exhaustive-rescan safety net — plus +11 in `geometry.test.ts` for the `t`-only ray helpers, incl. a deterministic case inside `raySegment`'s 1e-9 `u` tolerance band that 40 000 random rays never once hit). `|node|` (pure logic) + `|dom|` (jsdom + axe, **7** files) — see `vite.config.ts` `test.projects`. Ratchet: never let the count drop (95→126→140→181→239→245→296→322→340→613→644→649→655→659→666→711→760→814→961→**1082**). S20 added +147: `engine/__tests__/projects.test.ts` **39** (the folder model — every invariant that, if broken, loses work or makes it unreachable) · `projects-migration.test.ts` **18** (OLD-shape IDB records and an OLD-shape localStorage blob, SEEDED BY HAND and asserted on read, plus the export→import folder round-trip) · `compare/__tests__/compute-scenario.test.ts` **17** (the direct-only trace's exactness, with a negative control) · `compare-summary.test.ts` **20** (the N-way sentence for N = 0..MAX, and that it never states a conclusion about more columns than it measured) · **`column-gate.test.ts` 14 — the slow-column gate driven to a FIXED POINT over every cheap/slow mix up to eight columns, because the first version oscillated forever and no scene-level test could have seen it** · `compare.a11y.test.tsx` **10** + `gallery/__tests__/gallery.a11y.test.tsx` **7** (both PAGE-WIDE axe — the subtree run skips `landmark-unique`, which is exactly the defect N columns can introduce) · `hooks/__tests__/useProjectActions.test.tsx` **16** (the folder-CRUD orchestration — the toast that named the folder it had just deleted, the undo guard, and the shared undo slot all lived here) · `seed.test.ts` +6. S21 added +121 for the guided tour: `tutorial/__tests__/actions.test.ts` **17** — including THE ACCEPTANCE PROOF, which drives the real `traceScene`→`computeAudio` to show the tour reaches a lock, with an apartment negative control so the reason the practice room exists cannot be quietly forgotten, and the second-run re-arm · `machine.test.ts` **19** (the reducer treats a persisted chapter/step as UNTRUSTED input: clamp, never index) · `progress.test.ts` **13** (the gate, incl. the S16 trap — a returning IDB user with no localStorage keys must NOT be offered the tour) · `steps.test.ts` **22** — the COVERAGE test that enumerates `DIGIT_TOOL`/`MODE_THEME` and fails when a tool has no step, plus the stranding invariants and **the data-safety invariant that every scene-writing chapter is marked `needsPractice`** · `spotlight.test.ts` **16** (pure geometry + the `rectsEqual` anti-loop guard) · `tutorial.a11y.test.tsx` **20** (PAGE-WIDE axe on the menu, a narration step and a `try` step in both states) · `hooks/__tests__/useTutorial.test.tsx` **14** (the only place the tour writes — asserts every non-practice layout is byte-identical after the whole spine).
