...
by making the work cheaper rather than capping it — **50-room chain 13.7 s → 0.50 s (27×),
100-room 102.8 s → 1.8 s, object bomb 4.9 s → 0.12 s, wall-heavy span 399 129.7 s → 12.0 s**
...
in `docs/security.md` §"Worst-case CPU": the wall-heavy import-legal payload sits at **12.0 s**,
of which `computeAudio` is 8.5 s — `bestPairSpot` sweeping ~154 000 cells once per apex-blocked
