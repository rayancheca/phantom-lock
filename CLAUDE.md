...
- `npm test` — vitest, **913 tests** across TWO projects, all green as of 2026-07-28. (S19 reflection-search perf had added +54: `engine/__tests__/reflection.test.ts` 36 — the golden equivalence over 18 branch-coverage probe scenes, a brute-force ORACLE that transcribes the pre-S19 algorithm and fuzzes it against the new one over randomized scenes at origins 0/1e3/1e5, the `surfaces.length`-unchanged guard that protects S18's grid cap, and a negative-radius-circle test that forces `isBlocked`'s exhaustive-rescan safety net — plus +11 in `geometry.test.ts` for the `t`-only ray helpers, incl. a deterministic case inside `raySegment`'s 1e-9 `u` tolerance band that 40 000 random rays never once hit). `|node|` (pure logic) + `|dom|` (jsdom + axe, **5** files) — see `vite.config.ts` `test.projects`. Ratchet: never let the count drop (95→126→140→181→239→245→296→322→340→613→644→649→655→659→666→711→760→814→**913**). S20 added +99: `engine/__tests__/projects.test.ts` 30 (the folder model — every invariant that, if broken, loses work or makes it unreachable) · `projects-migration.test.ts` 13 (OLD-shape IDB records and an OLD-shape localStorage blob, SEEDED BY HAND and asserted on read) · `compare/__tests__/compute-scenario.test.ts` 17 (the direct-only trace's exactness, with a negative control) · `compare-summary.test.ts` 16 (the N-way sentence for N = 0..MAX) · `compare.a11y.test.tsx` 10 + `gallery/__tests__/gallery.a11y.test.tsx` 7 (both PAGE-WIDE axe — the subtree run skips `landmark-unique`, which is exactly the defect N columns can introduce) · `seed.test.ts` +6.
