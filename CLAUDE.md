...
- **SimCanvas is still over 800 lines** — **1042 after S37** (was 1447), and the two cuts that finish the job are specified in `docs/ideas.md` §18a: `pick.ts` (the twelve-branch pointerdown ladder, ~190 lines, and the valuable one — it is where the S36 grip-vs-pod hit-test ORDERING lives, currently an invariant with a comment and no test) and `chain.ts` (~55). S37 took out the drag transforms, the camera, the painter and the three DOM overlays.
